...
if [[ -z "${MESOS_SLAVE_PID}" ]]; then
    ...
    source ~/.chs_env/efs-submission-api/env

    PORT="${EFS_SUBMISSION_API_PORT}"
else
    PORT="$1"
    CONFIG_URL="$2"
    ENVIRONMENT="$3"
...
exec java ${JAVA_MEM_ARGS} -Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=4${PORT} -jar -Dserver.port="${PORT}" "${APP_DIR}/efs-submission-api.jar"

...
//    private FileTransferApiClient client;
     ...
     */
    public DecisionEngine(FileTransferService client, FormTemplateRepository repository,
    ...
    private void checkAvStatus(String submissionId, Decision decision, FileDetails fileDetails) {
        Optional<FileDetailsApi> response = this.client.getFileDetails(fileDetails.getFileId());
        if (response.isPresent()) {
            if (AV_FAILED.equals(response.get().getAvStatus().getValue())) {
                ...
                decision.incrementNumberOfDecisions();
            } else if (AV_CLEAN.equals(response.get().getAvStatus().getValue())) {
